                                                    ...
                                                    <div class="card mt-4 shadow">
                                                        <div class="card-body p-4">
                                                            ...
                                                            <form id="commentForm" action="{{ route('event.addComment', ['id' => $event->id]) }}" method="post">
                                                                @csrf
                                                                <div class="mb-3">
                                                                    <textarea class="form-control rounded" name="content" rows="5" placeholder="Your comment" required {{ $event->eventparticipants()->where('id_user', Auth::user()->id)->exists() ? '' : 'disabled' }}></textarea>
                                                                </div>
                                                                <button type="submit" class="btn btn-primary w-100 py-2" {{ $event->eventparticipants()->where('id_user', Auth::user()->id)->exists() ? '' : 'disabled' }}>Add Comment</button>
                                                            </form>
                                                        </div>
                                                    </div>
                                                </div>
